...
function FlyToHandler({ camera }) {
  const map = useMap()
  useEffect(() => {
    if (!camera) return
    map.flyTo([camera.lat, camera.lng], 14, { duration: 0.5 })
  }, [camera, map])
  return null
}

export default function MapView({ cameras, selectedCamera, loading }) {
  return (
    <div className="map-area">
      <MapContainer center={GUJARAT_CENTER} zoom={7} scrollWheelZoom={true}>
        ...
        />
        <FlyToHandler camera={selectedCamera} />
        <MarkerClusterGroup>
          {cameras.map((cam) => (
            <CircleMarker
              key={cam.id}
