...
- **Video Wall**: one tile per registered source, each with Start/Stop
  controls for its capture worker and a "Run ANPR" button
...
- "Run ANPR" fires a job and shows its initial status (`queued`) but
  doesn't poll for completion — for a real demo, either wait a beat before
  checking the Events tab, or add polling via `getAnprJob()` (already
  exported, just not wired to a timer yet).
