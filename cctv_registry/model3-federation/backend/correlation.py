...
def build_camera_context_map(cameras: list[dict]) -> dict:
    """
    Turns Model 1's camera list into a lookup by camera_identifier, for
    fast joining against Model 2's events (which reference cameras by ID
    but don't carry department/health data themselves).
    """
    context = {}
    for cam in cameras:
        cam_id = cam.get("camera_identifier")
        if cam_id:
            context[cam_id] = {
...
def correlate_events(raw_events: list[dict], camera_context: dict) -> list[dict]:
    ...
    for event in raw_events:
        cam_id = event.get("cameraId") or event.get("camera_id")
        context = camera_context.get(cam_id, {"department": None, "healthStatus": "unknown"})

        correlated.append(
            {
                "id": event.get("id"),
                "plateNumber": event.get("plateNumber") or event.get("plate_number"),
                "cameraId": cam_id,
